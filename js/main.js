...
let $nav_vue = new Vue({
    ...
    methods: {
        ...
        removeItem: function(e){
            ...
            let itemID = parseInt($(e.target).parent().attr('data-position-id'))
                itemrowID = '#'+$(e.target).parent().attr('data-element-id'),
                selectedItem = this.basket.items[itemID];
            ...
            */
            this.basketCount -= 1;
            this.basketTotal -= selectedItem.total_price;
            $(itemrowID).fadeOut();

            // delete from array
            this.basket.deletedItems.push(this.basket.items.splice(itemID,1));
